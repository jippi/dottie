...
// InputModel is the Bubble Tea model for this text input element.
...
// NewInput creates a new model with default settings.
...
func (m *InputModel) setValueInternal(runes []rune) {
	if m.Validate != nil {
...
func (m InputModel) Position() int {
	return m.pos
}
...
func (m *InputModel) SetSuggestions(suggestions []string) {
...
func (m *InputModel) san() runeutil.Sanitizer {
	if m.rsan == nil {
...
func (m *InputModel) insertRunesFromUserInput(v []rune) {
	// Clean up any special characters in the input provided by the
...
func (m *InputModel) handleOverflow() {
...
func (m *InputModel) deleteBeforeCursor() {
...
// masked input.
func (m *InputModel) deleteAfterCursor() {
	m.value = m.value[:m.pos]
	m.SetCursor(len(m.value))
}
...
// delete everything after the cursor so as not to reveal word breaks in the
// masked input.
func (m *InputModel) deleteWordForward() {
...
func (m *InputModel) currentWord() string {
	if len(m.value) == 0 {
		return ""
	}
	...
	start := m.pos
	for start > 0 {
		if unicode.IsSpace(m.value[start-1]) {
			break
		}
	...
	for end < len(m.value) {
		if unicode.IsSpace(m.value[end]) {
			break
		}
...
func (m *InputModel) currentWordQuoted() string {
	words := SafeSplitWords(string(m.value))

	for _, word := range words {
		if word.Start <= m.pos && m.pos <= word.Stop {
			return word.Value
		}
	}
...
func (m *InputModel) wordBackward() {
	if m.pos == 0 || len(m.value) == 0 {
		return
	}
...
func (m InputModel) echoTransform(v string) string {
	switch m.EchoMode {
	case EchoPassword:
...
func (m InputModel) placeholderView() string {
	var (
		v     string
...
// Deprecated: use cursor.Mode().
func (m InputModel) CursorMode() CursorMode {
	return CursorMode(m.Cursor.Mode())
}

// Deprecated: use cursor.SetMode().
func (m *InputModel) SetCursorMode(mode CursorMode) tea.Cmd {
	return m.Cursor.SetMode(cursor.Mode(mode))
}

func (m InputModel) completionView(offset int) string {
	var (
		value = m.value
		style = m.PlaceholderStyle.Inline(true).Render
	)
...
func (m *InputModel) AvailableSuggestions() []string {
...
func (m *InputModel) CurrentSuggestion() string {
...
func (m *InputModel) canAcceptSuggestion() bool {
...
func (m *InputModel) nextSuggestion() {
...
func (m *InputModel) previousSuggestion() {
	m.currentSuggestionIndex = (m.currentSuggestionIndex - 1)
