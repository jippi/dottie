...
func New(assignment *ast.Assignment) Model {
	return Model{
...
func (m *Model) validate(in string) error {
	m.Err = nil
...
func (m *Model) setValueInternal(runes []rune) {
	...
	m.Assignment.Literal = string(m.value)
...
func (m *Model) handleOverflow() {
	...
	if m.pos < m.offset {
		...
		for i < len(runes) && width <= m.Width {
			width += rw.RuneWidth(runes[i])
			if width <= m.Width+1 {
				i++
			}
		}
...
func (m *Model) deleteBeforeCursor() {
	m.setValueInternal(m.value[m.pos:])
	m.offset = 0
	m.SetCursor(0)
}
...
func (m *Model) deleteWordBackward() {
	...
	if oldPos > len(m.value) {
		...
		m.setValueInternal(append(m.value[:m.pos], m.value[oldPos:]...))
	}
}
...
func (m *Model) deleteWordForward() {
	...
	if m.pos > len(m.value) {
		m.setValueInternal(m.value[:oldPos])
	} else {
		m.setValueInternal(append(m.value[:oldPos], m.value[m.pos:]...))
	}
...
func (m Model) echoTransform(value string) string {
	switch m.EchoMode {
	case EchoPassword:
		return strings.Repeat(string(m.EchoCharacter), uniseg.StringWidth(value))
...
func (m Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	if ok && key.Matches(keyMsg, m.KeyMap.AcceptSuggestion) {
		if m.canAcceptSuggestion() {
			m.setValueInternal(append(m.value, m.matchedSuggestions[m.currentSuggestionIndex][len(m.value):]...))
			m.CursorEnd()
		}
	}
	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, m.KeyMap.DeleteCharacterBackward):
			if len(m.value) > 0 {
				m.setValueInternal(append(m.value[:max(0, m.pos-1)], m.value[m.pos:]...))
				m.SetCursor(m.pos)
			}
...
func (m Model) View() string {
	...
	text := styleText(m.echoTransform(string(value[:pos])))

	if pos < len(value) {
		char := m.echoTransform(string(value[pos]))
		m.Cursor.SetChar(char)
		text += m.Cursor.View()                                   // cursor and text under it
		text += styleText(m.echoTransform(string(value[pos+1:]))) // text after cursor
		text += m.completionView(0)                               // suggested completion
	} else {
		if m.canAcceptSuggestion() {
	...
	if m.Width > 0 && valWidth <= m.Width {
		...
		text += styleText(strings.Repeat(" ", padding))
	}

	return m.PromptStyle.Render(m.Prompt) + text
}
...
func (m Model) placeholderView() string {
	...
	if m.Width > 0 {
		...
		value += style(string(placeholder[1:minWidth]))
		value += style(strings.Repeat(" ", availWidth))
	} else {
		// if there is no width, the placeholder can be any length
		value += style(string(placeholder[1:]))
	}
