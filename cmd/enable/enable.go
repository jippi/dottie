...
func NewCommand() *cobra.Command {
	return &cobra.Command{
		...
		GroupID:     "manipulate",
		RunE:        runE,
		Annotations: map[string]string{comptplus.DynamicSuggestionsAnnotation: "enable"},
		ValidArgsFunction: shared.NewCompleter().
			WithSelectors(ast.ExcludeActiveAssignments).
			WithSettings(render.WithIncludeDisabled(true)).
			Get(),
	}
}
